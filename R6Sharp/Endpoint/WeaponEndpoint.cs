...
namespace R6Sharp.Endpoint
{
    public class WeaponEndpoint
    {
        ...
        public WeaponEndpoint(SessionEndpoint sessionHandler)
        {
        ...
        public async Task<DataResponse> GetWeaponAsync(Guid uuid, Gamemode gamemodes, Platform platforms, TeamRole teamroles, DateTime start, DateTime end)
        {
            ...
            var results = await ApiHelper.GetDataAsync(UbiconnectEndpoint.Weapon, uuid, queries, session).ConfigureAwait(false);
